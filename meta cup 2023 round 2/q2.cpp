...
map <pair<int,int>, int> complexAirhole;

pair<int,int> airholeLocation;
...
int countAirHoles(vector <vector<char>> chessBoard, vector <pair<int,int>> group){
    ...
    for (auto a: group){ // a is pair
        ...
        if (0 <= (a.second-1) && chessBoard[a.first][a.second-1] == '.' && find(dotBeenthere.begin(), dotBeenthere.end(), make_pair(a.first,a.second-1)) == dotBeenthere.end()){
            ...
            airholeLocation = make_pair(a.first,a.second-1);
        }
        //right
        ...
        if (0 <= (a.first-1) && chessBoard[a.first-1][a.second] == '.'&& find(dotBeenthere.begin(), dotBeenthere.end(), make_pair(a.first-1,a.second)) == dotBeenthere.end()){
            ...
            airholeLocation = make_pair(a.first-1,a.second);
        }
        //buttom
    ...
    if (count <2){
    	complexAirhole[airholeLocation] += group.size(); 
    }
...
int canCapture(vector <vector<char>> chessBoard){
...
*/
    vector <int> numberofCapture;
    for (auto a: groups){
        int airhole = countAirHoles(chessBoard,a);
        bool can = (airhole < 2) ? true: false;
        if (can){
        	numberofCapture.push_back(a.size());
        }
    }

    if (numberofCapture.size()== 0)return 0;

    int outcome = numberofCapture[0];

    //add complex airhole
    for (auto a: complexAirhole){
    	if (a.second > outcome){
    		outcome = a.second;
    	}
    }
...
int main(){
   ...
   vector <vector<char>> chessBoard; //row,column
    ...
    for(int x = 1; x <= numberOfCase; x++ ) {
        ...
        outputFile << "Case #" << x << ": " << canCapture(chessBoard) << endl;
        chessBoard.clear();
        groups.clear();
        beenThere.clear();
        complexAirhole.clear();
